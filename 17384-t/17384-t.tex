...
in'' $\alpha$; $A$, $B$, $C$ ``are points of'' $\alpha$, etc.
...
a plane $\alpha$, then every point of $a$ lies in $\alpha$}.
\end{description}
%%-----File: 014.png-----%%
...
plane $\alpha$, the totality of all points $A$, for which the segments
...
through $A$ does not meet the given straight line $a$; for, if
it should cut it, say in the point $D$, and if we suppose $B$ to be
situated between $C$ and $D$, we could then find on $a$ a
